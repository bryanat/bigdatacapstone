...
// Trend Six will triple the price for shoes purchased in China’
...
object trend6 {
  ...
  def manipulateTransactionTrend6(inputTransaction: String, counter: Int): String = {
    val splitT = inputTransaction.split(",")
    var resultString = ""
    if (counter == 10) {
      resultString = splitT(0) + "," + splitT(1) + "," + splitT(2) + "," + splitT(3) + "," + splitT(4) + "," + splitT(5) + "," + splitT(6) + "," +
        splitT(7) + "," + splitT(8) + "," + splitT(9)*3 + "," + splitT(10) + "," + "China" + "," + splitT(12) + ","+ splitT(13) + "," + splitT(14) + "," + splitT(15)
      return resultString
    }
  ...
  def getTrend6(spark: SparkSession, returnAmount: Int): Vector[String]={
    var orderCounter = 100000
    var orderID = trendTag+orderCounter.toString
    var repeatCounter = 1
    var resultList = ListBuffer("")
    for (i <- 0 to returnAmount) {
      val tempString = trans.createInitialTransaction(rs, spark, orderID,  "Shoes")
